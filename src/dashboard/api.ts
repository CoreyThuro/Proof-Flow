...
import { getRepoByOwnerAndName } from '../db/queries/repos';
import { getLatestSnapshot } from '../db/queries/dashboardSnapshots';

export async function dashboardApiHandler(req: Request, res: Response): Promise<void> {
  ...
  const snapshot = await getLatestSnapshot(repo.id);
  if (!snapshot) {
    res.json({
      repo: `${owner}/${name}`,
      rfc_open: 0,
...
export function dashboardUiHandler(req: Request, res: Response): void {
  const { owner, name } = req.params as { owner: string; name: string };
  res.setHeader('Content-Type', 'text/html');
  res.send(buildDashboardHtml(owner, name));
}

function buildDashboardHtml(owner: string, name: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
